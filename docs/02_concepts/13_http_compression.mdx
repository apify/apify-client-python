...
Pass `compression` to the client constructor to choose the compression algorithm:
...
:::tip
For most workloads, the bandwidth savings from brotli outweigh the small CPU overhead. Install the `brotli` extra and pass `compression='brotli'` unless you're running in an environment where installing additional packages isn't feasible.
:::
