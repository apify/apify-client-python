...
def test_actor_env_var_list(apify_client: ApifyClient) -> None:
    ...
    try:
        # List env vars
        env_vars = version_client.env_vars().list()
...
def test_actor_env_var_create_and_get(apify_client: ApifyClient) -> None:
    ...
    try:
        ...
        created_env_var = version_client.env_vars().create(
            name='MY_VAR',
            value='my_value',
            is_secret=False,
        )
        ...
        retrieved_env_var = env_var_client.get()
...
def test_actor_env_var_update(apify_client: ApifyClient) -> None:
    ...
    try:
        # Update the env var
        updated_env_var = env_var_client.update(
            name='UPDATE_VAR',
            value='updated_value',
        )
...
def test_actor_env_var_delete(apify_client: ApifyClient) -> None:
    ...
    try:
        ...
        # Verify it's gone
        deleted_env_var = env_var_client.get()
        assert deleted_env_var is None

        # Verify the other env var still exists
        remaining_env_var = version_client.env_var('VAR_TO_KEEP').get()
