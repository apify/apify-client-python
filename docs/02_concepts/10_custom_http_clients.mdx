...
and accepts fully custom synchronous or asynchronous implementations.
...
- The built-in Impit classes inherit directly from the corresponding sync or async class and adapt the underlying
  transport.
...
higher-level features such as streamed logs classify timeouts without depending on Impit or private implementation
details.
...
The built-in Impit classes are thin transport adapters over the request implementation in `HttpClient` and
...
The `@override` decorators in the built-in Impit adapters make these implementations explicit and allow type checkers
to catch misspelled or incompatible overrides.
...
Custom HTTP clients might be useful when the built-in Impit clients don't cover your requirements, for example when
you need to:

- **Use a different HTTP library** - Integrate [httpx](https://www.python-httpx.org/), [requests](https://requests.readthedocs.io/), [aiohttp](https://docs.aiohttp.org/), or another transport.
