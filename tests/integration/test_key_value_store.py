...
def test_key_value_store_should_create_expiring_keys_public_url_with_params_sync(
...
def test_key_value_store_should_create_public_keys_non_expiring_url_sync(apify_client: ApifyClient) -> None:
    created_store = apify_client.key_value_stores().get_or_create(name=random_resource_name('key-value-store'))
...
def test_public_url_sync(api_token: str, api_url: str, api_public_url: str, signing_key: str) -> None:
...
@pytest.mark.parametrize('signing_key', [None, 'custom-signing-key'])
@parametrized_api_urls
def test_record_public_url_sync(api_token: str, api_url: str, api_public_url: str, signing_key: str) -> None:
    apify_client = ApifyClient(token=api_token, api_url=api_url, api_public_url=api_public_url)
    key = 'some_key'
    kvs = apify_client.key_value_store(MOCKED_ID)
...
def test_list_keys_signature_sync(apify_client: ApifyClient, test_kvs_of_another_user: TestKvs) -> None:
...
def test_get_record_signature_sync(apify_client: ApifyClient, test_kvs_of_another_user: TestKvs) -> None:
    key = 'key1'
    kvs = apify_client.key_value_store(key_value_store_id=test_kvs_of_another_user.id)
...
def test_get_record_as_bytes_signature_sync(apify_client: ApifyClient, test_kvs_of_another_user: TestKvs) -> None:
    key = 'key1'
    kvs = apify_client.key_value_store(key_value_store_id=test_kvs_of_another_user.id)
...
def test_stream_record_signature_sync(apify_client: ApifyClient, test_kvs_of_another_user: TestKvs) -> None:
    key = 'key1'
    kvs = apify_client.key_value_store(key_value_store_id=test_kvs_of_another_user.id)
...
async def test_key_value_store_should_create_expiring_keys_public_url_with_params_async(
...
async def test_key_value_store_should_create_public_keys_non_expiring_url_async(
    apify_client_async: ApifyClientAsync,
) -> None:
    created_store = await apify_client_async.key_value_stores().get_or_create(
        name=random_resource_name('key-value-store')
...
@pytest.mark.parametrize('signing_key', [None, 'custom-signing-key'])
@parametrized_api_urls
async def test_public_url_async(api_token: str, api_url: str, api_public_url: str, signing_key: str) -> None:
...
@pytest.mark.parametrize('signing_key', [None, 'custom-signing-key'])
@parametrized_api_urls
async def test_record_public_url_async(api_token: str, api_url: str, api_public_url: str, signing_key: str) -> None:
    apify_client = ApifyClientAsync(token=api_token, api_url=api_url, api_public_url=api_public_url)
    key = 'some_key'
    kvs = apify_client.key_value_store(MOCKED_ID)
...
async def test_list_keys_signature_async(
...
async def test_get_record_signature_async(
    apify_client_async: ApifyClientAsync, test_kvs_of_another_user: TestKvs
) -> None:
    key = 'key1'
    kvs = apify_client_async.key_value_store(key_value_store_id=test_kvs_of_another_user.id)
...
async def test_get_record_as_bytes_signature_async(
    apify_client_async: ApifyClientAsync, test_kvs_of_another_user: TestKvs
) -> None:
    key = 'key1'
    kvs = apify_client_async.key_value_store(key_value_store_id=test_kvs_of_another_user.id)
...
async def test_stream_record_signature_async(
    apify_client_async: ApifyClientAsync, test_kvs_of_another_user: TestKvs
) -> None:
    key = 'key1'
    kvs = apify_client_async.key_value_store(key_value_store_id=test_kvs_of_another_user.id)
