...
async def test_actor_version_list(apify_client_async: ApifyClientAsync) -> None:
    ...
    try:
        # List versions
        versions = await actor_client.versions().list()
...
async def test_actor_version_create_and_get(apify_client_async: ApifyClientAsync) -> None:
    """Test creating and getting an actor version."""
    actor_name = get_random_resource_name('actor')

    # Create an actor without versions
    actor = await apify_client_async.actors().create(name=actor_name)
    actor_client = apify_client_async.actor(actor.id)

    try:
        # Create a new version
        created_version = await actor_client.versions().create(
            version_number='1.0',
        ...
        retrieved_version = await version_client.get()
...
async def test_actor_version_update(apify_client_async: ApifyClientAsync) -> None:
    ...
    try:
        # Update the version
        updated_version = await version_client.update(
            build_tag='updated',
            source_files=[
                {
                    'name': 'main.js',
...
async def test_actor_version_delete(apify_client_async: ApifyClientAsync) -> None:
    ...
    try:
        # Delete version 0.1
        version_client = actor_client.version('0.1')
        await version_client.delete()

        # Verify it's gone
        deleted_version = await version_client.get()
        assert deleted_version is None

        # Verify version 0.2 still exists
        remaining_version = await actor_client.version('0.2').get()
