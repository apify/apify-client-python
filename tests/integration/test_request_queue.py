...
async def test_request_queue_list_requests(client: ApifyClient | ApifyClientAsync, *, is_async: bool) -> None:
    ...
    try:
        ...
        list_response = cast('ListPage[Request] | None', None)
        for _ in range(5):
            await maybe_sleep(1, is_async=is_async)
            list_response = cast('ListPage[Request]', await maybe_await(rq_client.list_requests()))
...
async def test_request_queue_batch_delete_requests(client: ApifyClient | ApifyClientAsync, *, is_async: bool) -> None:
    ...
    try:
        ...
        remaining = cast('ListPage[Request] | None', None)
        for _ in range(5):
            await maybe_sleep(1, is_async=is_async)
            remaining = cast('ListPage[Request]', await maybe_await(rq_client.list_requests()))
