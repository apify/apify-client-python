...
async def main() -> None:
    ...
    producer_run = await apify_client.actor('my-org/producer-actor').call(
    ...
    async with producer_store.stream_record('OUTPUT') as record:
        if record is None:
            raise RuntimeError('The producer stored no OUTPUT record')
        consumer_run = await apify_client.actor('my-org/consumer-actor').call(
    ...
    reports_store = await apify_client.key_value_stores().get_or_create(
    ...
    async with producer_dataset.stream_items(item_format='csv') as items:
        await reports_client.set_record('items.csv', items, content_type='text/csv')
