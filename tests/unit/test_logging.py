...
def _wait_for_blocking_read(streamed_log: StreamedLogBase, *, timeout: float = 5) -> None:
    """Wait until a chunk has been buffered, which proves the reader reached the blocking read.

    A fixed sleep could end before the reader gets there. Async callers reach this through `asyncio.to_thread`, so
    the streaming task keeps the event loop while the wait runs.
    """
    deadline = time.monotonic() + timeout
    while not streamed_log._stream_buffer and time.monotonic() < deadline:
        time.sleep(0.01)
    assert streamed_log._stream_buffer, 'the log stream never reached its blocking read'
...
def test_streamed_log_sync_stop_unblocks_on_finite_stream_timeout(
    ...
    streamed_log.start()
    try:
        _wait_for_blocking_read(streamed_log)
...
"""How many plain status requests `call` makes before it starts the watcher: one per redirector it builds."""
...
def mock_api_failing_status_poll(httpserver: HTTPServer) -> None:
    ...
    # `call` completes both of the plain status requests that build its redirectors before it starts either of them,
    # and `wait_for_finish` only ever sends `waitForFinish`, so every later plain status request is a watcher poll.
    plain_status_requests = 0
    request_lock = threading.Lock()

    def _status_handler(request: Request) -> Response:
        nonlocal plain_status_requests
        ...
        with request_lock:
            plain_status_requests += 1
            is_watcher_poll = plain_status_requests > _POLL_FAILURE_SETUP_STATUS_REQUESTS
...
def test_sync_watcher_start_recovers_from_a_failed_poll() -> None:
    """`start` runs again after a failed poll ended the polling thread, with no `stop` in between."""
    logger = logging.getLogger('apify_client.tests.watcher_restart')
    run_client = Mock()
    run_client.get.side_effect = RuntimeError('Simulated status poll failure')
    run_client._http_client.is_timeout_error.return_value = False

    watcher = StatusMessageWatcher(run_client=run_client, to_logger=logger, check_period=timedelta(seconds=0))
    first_thread = watcher.start()
    first_thread.join(timeout=5)
    assert not first_thread.is_alive()

    second_thread = watcher.start()
    try:
...
@pytest.mark.usefixtures('propagate_stream_logs')
def test_streamed_log_sync_stop_reports_failing_tail_flush(
    caplog: LogCaptureFixture,
    httpserver: HTTPServer,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A tail the stream cut mid-character keeps its traceback even though the flush runs during a `stop`."""
    monkeypatch.setattr(StreamedLog, '_stop_timeout_s', 0.5)

    release_server = threading.Event()

    def _truncated_handler(_request: Request) -> Response:
        def generate_logs() -> Iterator[bytes]:
            # A complete marker whose message ends halfway through a multi-byte character, as a dropped connection
            # mid-chunk leaves it.
            yield b'2025-05-13T07:24:12.588Z ACTOR: \xe2\x82'
            release_server.wait(timeout=30)
    ...
    httpserver.expect_request(
        f'/v2/actor-runs/{_MOCKED_RUN_ID}/log', method='GET', query_string='stream=true&raw=true'
    ).respond_with_handler(_truncated_handler)
    _register_run_and_actor_endpoints(httpserver)
    ...
    with caplog.at_level(logging.DEBUG, logger=logger_name):
        try:
            ...
            # The flush only runs once the server lets the blocked read end, which is after `stop` returned.
            release_server.set()
            streaming_thread.join(timeout=5)
    ...
    assert any(
        ...
        and record.message == 'Log redirection stopped due to unexpected error:'
        for record in caplog.records
    ), f'The truncated tail was not reported. Got: {[record.message for record in caplog.records]}'


@pytest.mark.usefixtures('propagate_stream_logs')
async def test_streamed_log_async_stop_reports_failing_tail_flush(
    caplog: LogCaptureFixture,
    httpserver: HTTPServer,
) -> None:
    """A tail the stream cut mid-character keeps its traceback, and the cancellation `stop` raised still lands."""
    release_server = threading.Event()
    ...
    streaming_task = streamed_log.start()
    try:
        with caplog.at_level(logging.DEBUG, logger=logger_name):
            await asyncio.to_thread(_wait_for_blocking_read, streamed_log)
            await streamed_log.stop()
    finally:
        release_server.set()

    assert streaming_task.cancelled(), 'the flush failure swallowed the cancellation `stop` raised'
    assert any(
        record.levelno == logging.ERROR
        and record.exc_info is not None
        and record.message == 'Log redirection stopped due to unexpected error:'
